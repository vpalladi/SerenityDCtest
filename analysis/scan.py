...
class scan():
    def __init__(self, scanId, site, description='BER Scan', sort='rx', fromFile=False):
        ...
        print( self.scanId )
        print( type(self.scanId) )

        # get the scans from file
        if fromFile :
            with open( self.scanId + '/config.json' ) as cfg_file:
                cfg = json.load( cfg_file )
                for key, val in cfg.items():
                    if val['site'].replace('site', '') == site.replace('site', ''):
                        fileName = self.scanId + '/' + self.site + '/' + key + '.csv'
                        new = run.Run.fromFile( fileName )
                        #print(new.dict)
                        self.scans.append( new )
...
#            self.scans = self.loadScan( self.scanId, self.site )
    ...
    def loadScan( path, site, comment="" ):
        print( type(path) )
        scans = []
        with open( path + '/config.json' ) as config_file:
            ...
            for key, val in cfg.items():
                if val['site'].replace('site', '') == site.replace('site', ''):
                    fileName = path + '/' + site + '/' + key + '.csv'
                    scans.append( run.Run.fromFile(fileName) )
        return scans
...
class scan():
    ...
    def __init__(self, scanID, site, description='BER Scan', sort='rx', fromDB=True):
        ...
        self.scans = db.query( { 'timestamp' : scan } )
