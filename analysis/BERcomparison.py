...
for r in csvReader_1em7_1em9:
    csvReader_1em7_1em9_edges.append([float(r[0].split(',')[0].replace(
        '(', '')), float(r[1].split(',')[0].replace('(', ''))])

csvReader_1em9_1em9_edges = []
for r in csvReader_1em9_1em9:
    csvReader_1em9_1em9_edges.append([float(r[0].split(',')[0].replace(
        '(', '')), float(r[1].split(',')[0].replace('(', ''))])

diff = []
openings = []
for rm9, rm7 in zip(csvReader_1em9_1em9_edges, csvReader_1em7_1em9_edges):
    diff.append([(rm9[0] - rm7[0]) / abs(rm9[0]),
                 (rm9[1] - rm7[1]) / abs(rm9[1])])
    openings.append(
        ((rm9[1] - rm9[0]) - (rm7[1] - rm7[0])) / (rm9[1] - rm9[0]))
