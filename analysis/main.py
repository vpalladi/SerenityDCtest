...
# parser
parser = OptionParser()
parser.add_option("-s", "--scanPath", dest="scanPath",
                  help="The path to the scan.", type='string')
parser.add_option("-d", "--DC", dest="DC",
                  help="The config file for the specific scan.", type='string', default='0')
...
parser.add_option("-q", "--scanPathCompare", dest="scanPathCompare",
                  help="The config file for the copmare scan.", type='string', default='')
parser.add_option("-m", "--DCcompare", dest="DCcompare",
...
scanPathCompare = options.scanPathCompare
DCcompare = options.DCcompare
...
s = scan( scanPath, DC, scanDescription, sort=scanSorting )
...
fig1,ax1 = plt.subplots( 1, 2, sharey=True, gridspec_kw={'wspace': 0}, figsize=(16,9) )
s.getAllOpeningsPlot( ax1[0] )
s.getAllOpeningsPlotDiff( ax1[1] )
fig1.canvas.set_window_title( '(main) Openings @'+str(s.getDwell()[0]) )

if scanPathCompare != '' :
    sCompare = scan( scanPathCompare, DCcompare, scanDescriptionCompare, sort=scanSorting )
    s.compare( sCompare )
    fig2,ax2 = plt.subplots( 1, 2, sharey=True, gridspec_kw={'wspace': 0}, figsize=(16,9) )
    sCompare.getAllOpeningsPlot( ax2[0] )
    sCompare.getAllOpeningsPlotDiff( ax2[1] )
    fig1.canvas.set_window_title( '(compare) Openings @'+str(sCompare.getDwell()[0]) )
